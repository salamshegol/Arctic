...
local espTab = createTabButton("ESP", 0.12)
local miscTab = createTabButton("Misc", 0.22)
local otherTab = createTabButton("Other", 0.32)
local visualTab = createTabButton("Visual", 0.42)
local weaponTab = createTabButton("Weapon", 0.52)
...
-- Weapon Tab Controls
createCheckbox(weaponContent, "No Spread", 0.1, weaponSettings.NoSpreadEnabled, function(val) weaponSettings.NoSpreadEnabled = val end)
...
damageInput.Position = UDim2.new(0.1, 0, 0.65, 0)
...
damageInput.Changed:Connect(function()
    ...
    if damage then
        weaponSettings.CustomDamageValue = math.clamp(damage, 1, 1000)
        damageInputLabel.Text = "Damage: " .. weaponSettings.CustomDamageValue
        updateWeaponSettings()
    end
end)
...
local function updateWeaponSettings()
    ...
    for _, weapon in pairs(weapons:GetChildren()) do
        -- weapon может быть Tool или Model, ищем NumberValue Spread
        local spread = weapon:FindFirstChild("Spread")
        if spread and spread:IsA("NumberValue") then
            if weaponSettings.NoSpreadEnabled then
                spread.Value = 0
                -- Если внутри Spread есть дочерние NumberValue, тоже ставим их в 0
                for _, child in pairs(spread:GetChildren()) do
...
local function unhookCheats()
    ...
    weaponSettings.LockedAmmo = false
